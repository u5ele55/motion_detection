...
class CVMotionDetector(IMotionDetector):
    def __init__(self, height: int, width: int, *, capacity: int = 10, min_area: int = 50):
    ...
    def detect_motion(self, frame: np.ndarray, return_processed_frame: bool = False) -> np.ndarray:
        ...
        diff = cv2.dilate(diff, np.ones((19, 19)))
        thresh_frame = cv2.threshold(src=diff, thresh=15, maxval=255, type=cv2.THRESH_BINARY)[1]
        ...
        # sort by area
        result = sorted(result, key=lambda rect: (rect[2]-rect[0])*(rect[3]-rect[1]))
